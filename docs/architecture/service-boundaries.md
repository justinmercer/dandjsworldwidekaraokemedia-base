...
| HQ server | Authorized catalog metadata, future venue profiles, future host registration records, future synchronization manifests, future request coordination, reports | Host status reports and request web submissions when implemented | Live playback authority or emergency show continuity |
...
| Data type | Primary owner | Wave artifact | Notes |
| --- | --- | --- | --- |
| Song metadata | HQ server | `song-metadata.v1.schema.json`, `server/hq/database/migrations/0001_authorized_catalog.sql` | Metadata only. No media files or media URLs. |
