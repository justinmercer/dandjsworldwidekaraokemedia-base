...
foreach ($taskNumber in 433..577) {
  $taskId = 'KARA-{0:D3}' -f $taskNumber
  if ($backlog -match "- \[x\] ``$taskId``") {
    throw "Backlog task $taskId should remain unchecked after Wave 9A."
  }
}
...
Write-Host "Wave 9A smoke checks passed: $($requiredFiles.Count) required files present, KARA-001..432 checked, KARA-433..577 unchecked, and safety guardrails passed."
