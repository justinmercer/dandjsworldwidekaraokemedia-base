...
foreach ($taskNumber in 448..577) {
  $taskId = 'KARA-{0:D3}' -f $taskNumber
  if ($backlog -match "- \[x\] ``$taskId``") {
    throw "Backlog task $taskId should remain unchecked after Wave 9B."
  }
}
...
Write-Host "Wave 9B smoke checks passed: $($requiredFiles.Count) required files present, KARA-001..447 checked, KARA-448..577 unchecked, and safety guardrails passed."
